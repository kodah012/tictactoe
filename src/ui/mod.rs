...
fn spawn_turn_text(
    ...
    let x_turn = commands.spawn(SpriteSheetBundle {
        texture_atlas: tex_atlas_handle.0.clone_weak(),
        sprite: TextureAtlasSprite::new(tex_atlas_indices.x_turn),
        transform: Transform::from_scale(Vec3::splat(8.))
            .with_translation(Vec3::new(0., 285., -99.)),
        visibility: Visibility::Hidden,
        ..default()
    })
        .insert(Name::new("X Turn"))
        .id();

    let o_turn = commands.spawn(SpriteSheetBundle {
        texture_atlas: tex_atlas_handle.0.clone_weak(),
